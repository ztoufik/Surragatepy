...
class Scalar_Poly_Expanser:
    def __init__(self, number_joint_RV,poly_order,quadrature_intg_order):
        ...
        self.iid_joint_dist=cp.Iid(cp.Uniform(0,1),self.number_joint_RV)# Hash don't change if you change the distribution
        self.nodes_file_name=f"nodes{hash((self.quadrature_intg_order,self.number_joint_RV))}.pi"
        self.expansions_file_name=f"expansions{hash((self.poly_order,self.number_joint_RV))}.pi"
